...
public class OfflineMessageManager {
    ...
    public boolean supportsFlexibleRetrieval() throws XMPPException {
        return ServiceDiscoveryManager.getInstanceFor(this.connection).discoverInfo((String) null).containsFeature(namespace);
    }

    public int getMessageCount() throws XMPPException {
        Form formFrom = Form.getFormFrom(ServiceDiscoveryManager.getInstanceFor(this.connection).discoverInfo((String) null, namespace));
        if (formFrom != null) {
    ...
    public Iterator<OfflineMessageHeader> getHeaders() throws XMPPException {
        ...
        Iterator<DiscoverItems.Item> items = ServiceDiscoveryManager.getInstanceFor(this.connection).discoverItems((String) null, namespace).getItems();
        while (items.hasNext()) {
    ...
    public Iterator<Message> getMessages(final List<String> list) throws XMPPException {
        ...
        for (String item : list) {
            ...
            item2.setAction(Promotion.ACTION_VIEW);
        ...
        if (iq == null) {
            ...
            for (Message message = (Message) createPacketCollector2.nextResult((long) SmackConfiguration.getPacketReplyTimeout()); message != null; message = (Message) createPacketCollector2.nextResult((long) SmackConfiguration.getPacketReplyTimeout())) {
