...
public class LottieAnimationView extends AppCompatImageView {
    ...
    private void enableOrDisableHardwareLayer() {
        int layerType;
        int renderModeIndex = AnonymousClass7.$SwitchMap$com$airbnb$lottie$RenderMode[this.renderMode.ordinal()];
        if (renderModeIndex == 1) {
            layerType = View.LAYER_TYPE_HARDWARE;
        } else if (renderModeIndex == 2) {
            layerType = View.LAYER_TYPE_SOFTWARE;
        } else {
            boolean useHardware = true;
            LottieComposition lottieComposition = this.composition;
            if (lottieComposition != null) {
                if (lottieComposition.hasDashPattern() && Build.VERSION.SDK_INT < 28) {
                    useHardware = false;
                }
                if (lottieComposition.getMaskAndMatteCount() > 4) {
                    useHardware = false;
                }
            }
            ...
            layerType = useHardware ? View.LAYER_TYPE_HARDWARE : View.LAYER_TYPE_SOFTWARE;
        }
