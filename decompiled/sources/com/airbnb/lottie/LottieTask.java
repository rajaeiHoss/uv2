...
public class LottieTask<T> {
        ...
        this.handler.post(new Runnable() {
            public void run() {
                if (LottieTask.this.result != null) {
                    LottieResult<T> access$000 = LottieTask.this.result;
    ...
    public synchronized void notifySuccessListeners(T t) {
        for (LottieListener<T> onResult : new ArrayList<>(this.successListeners)) {
            onResult.onResult(t);
        }
    }
    ...
    public synchronized void notifyFailureListeners(Throwable th) {
        ...
        for (LottieListener<Throwable> onResult : arrayList) {
