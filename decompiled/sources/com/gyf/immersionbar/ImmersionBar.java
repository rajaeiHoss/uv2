...
public final class ImmersionBar implements ImmersionCallback {
    ...
    private int hideBar(int i) {
        if (Build.VERSION.SDK_INT >= 16) {
            ...
            if (i2 == 1) {
                ...
                i |= View.SYSTEM_UI_FLAG_LAYOUT_HIDE_NAVIGATION | View.SYSTEM_UI_FLAG_HIDE_NAVIGATION;
            } else if (i2 == 4) {
                i |= 0;
            }
        }
    ...
    private void transformView() {
        if (this.mBarParams.viewMap.size() != 0) {
            for (Map.Entry<View, Map<Integer, Integer>> next : this.mBarParams.viewMap.entrySet()) {
    ...
    public static void setTitleBar(Activity activity, final int i, View... viewArr) {
        if (Build.VERSION.SDK_INT >= 19 && activity != null) {
            ...
            final int targetInset = statusBarHeight;
            for (final View view : viewArr) {
                if (view != null) {
                    Integer num = (Integer) view.getTag(R.id.immersion_fits_layout_overlap);
                    int numValue = num != null ? num.intValue() : 0;
                    if (numValue != targetInset) {
                        view.setTag(R.id.immersion_fits_layout_overlap, Integer.valueOf(targetInset));
                        ViewGroup.LayoutParams layoutParams = view.getLayoutParams();
                        if (layoutParams == null) {
                            layoutParams = new ViewGroup.LayoutParams(-1, -2);
                        }
                        final ViewGroup.LayoutParams finalLayoutParams = layoutParams;
                        final int finalNumValue = numValue;
                        if (finalLayoutParams.height == -2 || finalLayoutParams.height == -1) {
                            view.post(new Runnable() {
                                public void run() {
                                    finalLayoutParams.height = (view.getHeight() + targetInset) - finalNumValue;
                                    view.setPadding(view.getPaddingLeft(), (view.getPaddingTop() + targetInset) - finalNumValue, view.getPaddingRight(), view.getPaddingBottom());
                                    view.setLayoutParams(finalLayoutParams);
                                }
                            });
                        } else {
                            finalLayoutParams.height += targetInset - finalNumValue;
                            view.setPadding(view.getPaddingLeft(), (view.getPaddingTop() + targetInset) - finalNumValue, view.getPaddingRight(), view.getPaddingBottom());
                            view.setLayoutParams(finalLayoutParams);
                        }
                    }
