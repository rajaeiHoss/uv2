...
public final class Encoder {
    ...
    public static void encode(String str, ErrorCorrectionLevel errorCorrectionLevel, QRCode qRCode) throws WriterException {
        encode(str, errorCorrectionLevel, (Hashtable) null, qRCode);
    }
            ...
            if (i5 == i7) {
                for (int i12 = 0; i12 < i8; i12++) {
                    for (int i13 = 0; i13 < vector.size(); i13++) {
                        byte[] dataBytes = ((BlockPair) vector.elementAt(i13)).getDataBytes();
                        if (i12 < dataBytes.length) {
                            bitArray3.appendBits(dataBytes[i12], 8);
                        }
                    }
                }
                for (int i14 = 0; i14 < i9; i14++) {
                    for (int i15 = 0; i15 < vector.size(); i15++) {
                        byte[] errorCorrectionBytes = ((BlockPair) vector.elementAt(i15)).getErrorCorrectionBytes();
                        if (i14 < errorCorrectionBytes.length) {
                            bitArray3.appendBits(errorCorrectionBytes[i14], 8);
                        }
                    }
                }
