...
public final class zzhz {
    ...
    private static void zza(int i, long j, String str, int i2, PriorityQueue<zzia> priorityQueue) {
        zzia zzia = new zzia(j, str, i2);
        if ((priorityQueue.size() != i || (priorityQueue.peek().zzbal <= zzia.zzbal && priorityQueue.peek().value <= zzia.value)) && !priorityQueue.contains(zzia)) {
            priorityQueue.add(zzia);
    ...
    public static void zza(String[] strArr, int i, int i2, PriorityQueue<zzia> priorityQueue) {
        ...
        for (int i4 = 1; i4 < (strArr2.length - i3) + 1; i4++) {
            ...
            zzb = (((((j - ((((((long) zzhw.zzaa(strArr2[i4 - 1])) + TTL.MAX_VALUE) % 1073807359) * zza) % 1073807359)) % 1073807359) * 16785407) % 1073807359) + ((((long) zzhw.zzaa(strArr2[(i4 + i3) - 1])) + TTL.MAX_VALUE) % 1073807359)) % 1073807359;
            zza(i, zzb, zza(strArr2, i4, i3), strArr2.length, priorityQueue);
        }
    }

    private static long zzb(String[] strArr, int i, int i2) {
        long zzaa = (((long) zzhw.zzaa(strArr[0])) + TTL.MAX_VALUE) % 1073807359;
        for (int i3 = 1; i3 < i2; i3++) {
            zzaa = (((zzaa * 16785407) % 1073807359) + ((((long) zzhw.zzaa(strArr[i3])) + TTL.MAX_VALUE) % 1073807359)) % 1073807359;
        }
        return zzaa;
    }
}
