...
public final class zzflj {
    ...
    public final int zzcym() throws IOException {
        ...
        int b = zzcyv & ByteCompanionObject.MAX_VALUE;
        byte zzcyv2 = zzcyv();
        if (zzcyv2 >= 0) {
            i = zzcyv2 << 7;
        } else {
            b |= (zzcyv2 & ByteCompanionObject.MAX_VALUE) << 7;
            byte zzcyv3 = zzcyv();
            if (zzcyv3 >= 0) {
                i = zzcyv3 << 14;
            } else {
                b |= (zzcyv3 & ByteCompanionObject.MAX_VALUE) << 14;
                byte zzcyv4 = zzcyv();
                if (zzcyv4 >= 0) {
                    i = zzcyv4 << 21;
                } else {
                    int b2 = b | ((zzcyv4 & ByteCompanionObject.MAX_VALUE) << 21);
                    byte zzcyv5 = zzcyv();
                    int b3 = b2 | (zzcyv5 << 28);
                    if (zzcyv5 >= 0) {
                        return b3;
    ...
    public final long zzcyr() throws IOException {
        ...
        for (int i = 0; i < 64; i += 7) {
            byte zzcyv = zzcyv();
            j |= ((long) (zzcyv & ByteCompanionObject.MAX_VALUE)) << i;
            if ((zzcyv & ByteCompanionObject.MIN_VALUE) == 0) {
                return j;
            }
        }
        throw zzflr.zzdcp();
    ...
    public final int zzcys() throws IOException {
        return (zzcyv() & UByte.MAX_VALUE) | ((zzcyv() & UByte.MAX_VALUE) << 8) | ((zzcyv() & UByte.MAX_VALUE) << 16) | ((zzcyv() & UByte.MAX_VALUE) << 24);
    }
