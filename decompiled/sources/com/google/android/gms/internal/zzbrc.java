...
final class zzbrc extends Handler {
    ...
    zzbrc(Looper looper, Context context) {
        super(looper);
    ...
    public final void handleMessage(Message message) {
        if (message.what != 1) {
            zzbra.zzgpv.zzd("EventCallback", "Don't know how to handle this event in context %s", this.mContext);
            return;
        }
        ...
        if (type == 1) {
            ...
            DataHolder zzaqe = zzo.zzaqe();
            if (zzaqe != null) {
                zzq.zza(new zzbrd(new MetadataBuffer(zzaqe)));
            }
            if (zzo.zzaqf()) {
                zzq.zzcs(zzo.zzaqg());
            }
            ...
            zzbra.zzgpv.zzc("EventCallback", "Unexpected event: %s", driveEvent);
        } else {
            ((zzl) zzi).zza(new zzblt(((zzr) driveEvent).zzaqh()));
        }
    }
}
