...
class zzfjy<K extends Comparable<K>, V> extends AbstractMap<K, V> {
    ...
    public List<zzfkd<K, V>> zzpsl;
    /* access modifiers changed from: private */
    public Map<K, V> zzpsm;
    private volatile zzfkf<K, V> zzpsn;
    ...
    private final int zza(K k) {
        ...
        if (size >= 0) {
            int compareTo = k.compareTo(this.zzpsl.get(size).getKey());
            if (compareTo > 0) {
                return -(size + 2);
            }
            if (compareTo == 0) {
        ...
        while (i <= size) {
            int i2 = (i + size) / 2;
            int compareTo2 = k.compareTo(this.zzpsl.get(i2).getKey());
            if (compareTo2 < 0) {
                size = i2 - 1;
    ...
    public final V zzms(int i) {
        ...
        if (!this.zzpsm.isEmpty()) {
            ...
            this.zzpsl.add(new zzfkd<>(this, (Map.Entry<K, V>) it.next()));
            it.remove();
        }
        return value;
    }
    ...
    public boolean containsKey(Object obj) {
        K key = (K) obj;
        return zza(key) >= 0 || this.zzpsm.containsKey(key);
    }
    ...
    public V get(Object obj) {
        K key = (K) obj;
        int zza = zza(key);
        return zza >= 0 ? this.zzpsl.get(zza).getValue() : this.zzpsm.get(key);
    }
    ...
    public final V put(K k, V v) {
        ...
        int i2 = this.zzpsk;
        if (size == i2) {
            zzfkd<K, V> remove = this.zzpsl.remove(i2 - 1);
            zzdbs().put(remove.getKey(), remove.getValue());
        }
