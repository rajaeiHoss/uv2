...
public final class zzedo<K, V> extends zzedq<K, V> {
    ...
    public static <A, B, C> zzedo<A, C> zza(List<A> list, Map<B, C> map, zzedt<A, B> zzedt, Comparator<A> comparator) {
        ...
        return new zzedo<>(comparator, (A[]) objArr, (C[]) objArr2);
    }
    ...
    public final zzedq<K, V> zzg(K k, V v) {
        ...
        if (zzbn != -1) {
            K[] kArr = this.zzmav;
            if (kArr[zzbn] == k && this.zzmyg[zzbn] == v) {
                return this;
            }
            return new zzedo(this.zzmyh, zzb(kArr, zzbn, k), zzb(this.zzmyg, zzbn, v));
            ...
            while (true) {
                K[] kArr2 = this.zzmav;
                if (i < kArr2.length) {
                    hashMap.put(kArr2[i], this.zzmyg[i]);
                    i++;
                } else {
                    hashMap.put(k, v);
