...
public final class zzcia extends zzclh {
    ...
    public final void zzaj(long j) {
        zzcmd zzbcg = zzayh().zzbcg();
        for (String next : this.zzjff.keySet()) {
            zza(next, j - this.zzjff.get(next).longValue(), (zzclz) zzbcg);
        }
        if (!this.zzjff.isEmpty()) {
            zza(j - this.zzjfh, (zzclz) zzbcg);
        }
        zzak(j);
    }
