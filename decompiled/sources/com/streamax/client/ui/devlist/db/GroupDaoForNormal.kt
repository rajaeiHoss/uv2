...
class GroupDaoForNormal : GroupDao() {
    ...
    override fun add(str: String, i: Int, str2: String, i2: Int, i3: Int): Boolean {
        ...
        val insert = writableDatabase.insert(GroupDbBean.TableName0, null, contentValues)
    ...
    override fun delete(str: String): Boolean {
        val writableDatabase = mHelper.writableDatabase
        val delete = writableDatabase.delete(GroupDbBean.TableName0, "dbGroupName=?", arrayOf(str))
        writableDatabase.close()
        return delete > 0
    }
    ...
    override fun delete(i: Int): Boolean {
        val writableDatabase = mHelper.writableDatabase
        val delete = writableDatabase.delete(GroupDbBean.TableName0, "dbId=?", arrayOf("$i"))
        writableDatabase.close()
        return delete > 0
    }
    ...
    override fun query(str: String): Boolean =
        queryIsEmpty("dbGroupName=?", arrayOf(str))

    override fun query(str: String, i: Int): Boolean =
        queryIsEmpty("dbGroupName=? and dbFlag=?", arrayOf(str, "$i"))

    override fun queryDevName(str: String): Boolean =
        queryIsEmpty("dbDevName=?", arrayOf(str))
    ...
    override fun queryByGroupAndDevName(str: String, str2: String, i: Int): Boolean =
        queryIsEmpty("dbGroupName=? and dbDevName=? and dbChannel=?", arrayOf(str, str2, "$i"))
    ...
    override fun getGroupDatasByName(str: String): List<GroupBean> {
        ...
        val readableDatabase = mHelper.readableDatabase
        val rawQuery = readableDatabase.rawQuery("select * from GroupForNormal where dbGroupName =?", arrayOf(str))
        if (rawQuery != null) {
            while (rawQuery.moveToNext()) {
                arrayList.add(rawQuery.toGroupBean())
            }
            rawQuery.close()
        }
    ...
    override fun getGroupDatas(): List<GroupBean> {
        val arrayList = ArrayList<GroupBean>()
        val readableDatabase = mHelper.readableDatabase
        val rawQuery = readableDatabase.rawQuery("select * from GroupForNormal", null)
        if (rawQuery != null) {
            while (rawQuery.moveToNext()) {
                arrayList.add(rawQuery.toGroupBean())
            }
    ...
    private fun queryIsEmpty(selection: String, selectionArgs: Array<String>): Boolean {
        val readableDatabase = mHelper.readableDatabase
        val query = readableDatabase.query(GroupDbBean.TableName0, null, selection, selectionArgs, null, null, "id asc")
