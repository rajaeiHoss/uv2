...
public class ViewPager extends ViewGroup {
    ...
    public void populate(int newCurrentItem) {
        ItemInfo oldCurInfo;
        int oldCurPosition = this.mCurItem;
        if (oldCurPosition != newCurrentItem) {
            oldCurInfo = infoForPosition(oldCurPosition);
        ...
        int endPos = Math.min(adapterCount - 1, this.mCurItem + pageLimit);
        if (adapterCount != this.mExpectedAdapterCount) {
            ...
            throw new IllegalStateException("The application's PagerAdapter changed the adapter's contents without calling PagerAdapter#notifyDataSetChanged! Expected adapter item count: " + this.mExpectedAdapterCount + ", found: " + adapterCount + " Pager id: " + resName + " Pager class: " + getClass() + " Problematic adapter: " + this.mAdapter.getClass());
        }
        int curIndex = 0;
        ...
        if (curItem != null) {
            ...
            int clientWidth = getClientWidth();
            float leftWidthNeeded = clientWidth > 0 ? (2.0f - curItem.widthFactor) + (((float) getPaddingLeft()) / ((float) clientWidth)) : 0.0f;
            ...
            while (pos >= 0) {
                if (extraWidthLeft >= leftWidthNeeded && pos < startPos) {
                    ...
                    ItemInfo newItem = addNewItem(pos, itemIndex + 1);
                    extraWidthLeft += newItem.widthFactor;
                    curIndex++;
                    ii = itemIndex >= 0 ? this.mItems.get(itemIndex) : null;
                }
                pos--;
            ...
            ItemInfo ii2 = itemIndex2 < this.mItems.size() ? this.mItems.get(itemIndex2) : null;
            float rightWidthNeeded = clientWidth > 0 ? (((float) getPaddingRight()) / ((float) clientWidth)) + 2.0f : 0.0f;
            int pos2 = this.mCurItem + 1;
            while (pos2 < adapterCount) {
                if (extraWidthRight >= rightWidthNeeded && pos2 > endPos) {
                    if (ii2 == null) {
                        break;
                    }
                    if (pos2 == ii2.position && !ii2.scrolling) {
                        this.mItems.remove(itemIndex2);
                        this.mAdapter.destroyItem((ViewGroup) this, pos2, ii2.object);
                        ii2 = itemIndex2 < this.mItems.size() ? this.mItems.get(itemIndex2) : null;
                    }
                } else if (ii2 != null && pos2 == ii2.position) {
                    extraWidthRight += ii2.widthFactor;
                    itemIndex2++;
                    ii2 = itemIndex2 < this.mItems.size() ? this.mItems.get(itemIndex2) : null;
                } else {
                    ItemInfo newItem2 = addNewItem(pos2, itemIndex2);
                    itemIndex2++;
                    extraWidthRight += newItem2.widthFactor;
                    ii2 = itemIndex2 < this.mItems.size() ? this.mItems.get(itemIndex2) : null;
                }
                pos2++;
            }
        ...
        if (hasFocus()) {
            ...
            ItemInfo ii4 = currentFocused != null ? infoForAnyChild(currentFocused) : null;
            if (ii4 == null || ii4.position != this.mCurItem) {
                for (int i = 0; i < getChildCount(); i++) {
                    View child = getChildAt(i);
                    ItemInfo ii5 = infoForChild(child);
                    if (ii5 != null && ii5.position == this.mCurItem && child.requestFocus(2)) {
                        break;
                    }
                }
            }
    ...
    public static class SavedState extends AbsSavedState {
        ...
        public String toString() {
            return "FragmentPager.SavedState{" + Integer.toHexString(System.identityHashCode(this)) + " position=" + this.position + Constants.JsonSstringSuffix;
        }
    ...
    public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        while (i < count) {
            ...
            if (child.getVisibility() != 8) {
                ...
                if (lp != null && lp.isDecor) {
                    ...
                    int childWidthSpec = View.MeasureSpec.makeMeasureSpec(widthSize, childWidthMode);
                    int childHeightSpec = View.MeasureSpec.makeMeasureSpec(heightSize, childHeightMode);
        ...
        for (int childIndex = 0; childIndex < childCount; childIndex++) {
            ...
            if (child.getVisibility() != 8) {
                LayoutParams lp = (LayoutParams) child.getLayoutParams();
                if (lp == null || !lp.isDecor) {
                    int childWidthSpec = View.MeasureSpec.makeMeasureSpec((int) (((float) childWidthSize) * lp.widthFactor), 1073741824);
                    child.measure(childWidthSpec, this.mChildHeightMeasureSpec);
                }
            }
        }
    }
        ...
        while (childIndex < count) {
            ...
            if (child.getVisibility() != 8) {
                ...
                if (!lp.isDecor) {
                    ...
                    if (ii != null) {
                        int childLeft = paddingLeft + ((int) (((float) childWidth) * ii.offset));
                        if (lp.needsMeasure) {
                            lp.needsMeasure = false;
                            int widthSpec = View.MeasureSpec.makeMeasureSpec((int) (((float) childWidth) * lp.widthFactor), 1073741824);
    ...
    public void onPageScrolled(int position, float offset, int offsetPixels) {
        if (this.mDecorChildCount > 0) {
            ...
            for (int i = 0; i < childCount; i++) {
                ...
                if (lp.isDecor) {
                    ...
                    int newLeft = childLeft + scrollX;
                    int delta = newLeft - child.getLeft();
                    if (delta != 0) {
        ...
        if (this.mPageTransformer != null) {
            ...
            for (int i = 0; i < childCount2; i++) {
                ...
                if (!lp2.isDecor) {
                    float transformPos = ((float) (child2.getLeft() - scrollX2)) / ((float) getClientWidth());
