...
public abstract class FragmentStateAdapter extends RecyclerView.Adapter<FragmentViewHolder> implements StatefulAdapter {
    ...
    private void scheduleGracePeriodEnd() {
        ...
        final Runnable gracePeriodEnd = new Runnable() {
            public void run() {
        ...
        this.mLifecycle.addObserver(new LifecycleEventObserver() {
            public void onStateChanged(LifecycleOwner lifecycleOwner, Lifecycle.Event event) {
                if (event == Lifecycle.Event.ON_DESTROY) {
                    handler.removeCallbacks(gracePeriodEnd);
                    lifecycleOwner.getLifecycle().removeObserver(this);
                }
            }
        });
        handler.postDelayed(gracePeriodEnd, 10000);
    }
    ...
    class FragmentMaxLifecycleEnforcer {
        ...
        public void register(RecyclerView recyclerView) {
            ...
            this.mViewPager.registerOnPageChangeCallback(pageChangeCallback);
            DataSetChangeObserver dataObserver = new DataSetChangeObserver() {
                public void onChanged() {
                    FragmentMaxLifecycleEnforcer.this.updateFragmentMaxLifecycle(true);
                }
            };
            this.mDataObserver = dataObserver;
            FragmentStateAdapter.this.registerAdapterDataObserver(dataObserver);
